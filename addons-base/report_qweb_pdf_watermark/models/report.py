...
class Report(models.Model):
    ...
    def _render_qweb_pdf(self, report_ref, res_ids=None, data=None):
        ...
        if not self.env.context.get("res_ids"):
            _self = self.with_context(res_ids=res_ids)
        #     return super(Report, self.with_context(res_ids=res_ids))._render_qweb_pdf(
        #         res_ids=res_ids, data=data
        #     )
        return super(Report, _self)._render_qweb_pdf(report_ref=report_ref, res_ids=res_ids, data=data)
    ...
    def _run_wkhtmltopdf(
        ...
        if report_id.pdf_watermark:
            watermark = b64decode(report_id.pdf_watermark)
        elif report_id.use_company_watermark and report_id.env.company.pdf_watermark:
            print(3333)
            watermark = b64decode(self.env.company.pdf_watermark)
        elif docids:

            watermark = safe_eval(
                report_id.pdf_watermark_expression or "None",
                dict(env=self.env, docs=self.env[report_id.model].browse(docids)),
                )
            if watermark:
        ...
        try:
            ...
            try:
                ...
                resolution = image.info.get("dpi", report_id.paperformat_id.dpi or 90)
